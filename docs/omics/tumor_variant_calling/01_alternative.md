...
for file in $fastq_dir/*_R1.fastq; do
    ...
    trim_galore --paired --output_dir $output_dir/trimmed_data $fastq_dir/${base}_R1.fastq $fastq_dir/${base}_R2.fastq
done
...
for file in $output_dir/trimmed_data/*_val_1.fq; do
    ...
    bwa mem $reference_genome $output_dir/trimmed_data/${base}_val_1.fq $output_dir/trimmed_data/${base}_val_2.fq > $output_dir/aligned_data/${base}.sam
done
...
for file in $output_dir/aligned_data/*.sam; do
    ...
    samtools sort -o $output_dir/sorted_data/${base}.bam $output_dir/aligned_data/${base}.sam
...
for file in $output_dir/sorted_data/*.bam; do
    ...
    java -jar picard.jar MarkDuplicates I=$output_dir/sorted_data/${base}.bam O=$output_dir/deduplicated_data/${base}_dedup.bam M=$output_dir/deduplicated_data/${base}_dedup_metrics.txt
...
for file in $output_dir/deduplicated_data/*_dedup.bam; do
    ...
    gatk BaseRecalibrator -R $reference_genome -I $output_dir/deduplicated_data/${base}_dedup.bam -O $output_dir/recalibrated_data/${base}_recal_data.table --known-sites known_sites.vcf
done
...
for file in $output_dir/recalibrated_data/*_recal_data.table; do
    ...
    gatk ApplyBQSR -R $reference_genome -I $output_dir/deduplicated_data/${base}_dedup.bam -O $output_dir/final_data/${base}_recal.bam --bqsr-recal-file $output_dir/recalibrated_data/${base}_recal_data.table
done
...
for file in $output_dir/final_data/*_recal.bam; do
    ...
    gatk Mutect2 -R $reference_genome -I $output_dir/final_data/${base}_recal.bam -O $output_dir/variant_calls/${base}_variants.vcf
done
...
for file in $output_dir/variant_calls/*.vcf; do
    ...
    gatk Funcotator -R $reference_genome -V $output_dir/variant_calls/${base}_variants.vcf -O $output_dir/annotated_variants/${base}_annotated.vcf
done
